-- ============================================================
-- Migration 079 · Freigabe-Audit-Log
--
-- Protokolliert jede Statusänderung eines Raum-Produkts im
-- Freigabe-Kontext. Wird von Portal-Freigabe, Token-Freigabe,
-- Admin-Zurücksetzen und Auto-Invalidierung (bei Produkt-Änderung
-- nach bereits erfolgter Freigabe) beschrieben.
--
-- Dient als Audit-Trail für Admin-UI (Drawer pro Token zeigt
-- History) und erfüllt Compliance-Anforderungen bei Kunden, die
-- nachvollziehen müssen, wer wann was freigegeben hat.
-- ============================================================
...
-- Portal/Token-Flows laufen über createAdminClient (bypasses RLS),
-- deshalb keine anon-Policies nötig.
...
COMMENT ON COLUMN freigabe_audit.geaendert_von IS
  'Freitext-Name: bei Portal session-email, bei Token der im Abschluss eingegebene Name, bei Admin der User-Name, bei System "system".';
