...
- S31: Multi-Tenancy-Grundlage (Migration 036): Tabelle `organisationen` (Abo-Pläne trial/starter/professional/enterprise, max_projekte/max_mitglieder), `organisation_id` (nullable) auf 24 Tabellen, Indizes auf 5 Kerntabellen, Helper-Funktion `get_user_org_id()` (SECURITY DEFINER), alle bestehenden RLS-Policies gedroppt und durch org-scoped Policies ersetzt, Anon-Policy für freigabe_tokens erhalten, Admin-only Update-Policy für organisationen, Tabelle `audit_log` mit RLS. Code-Integration: `getOrganisationId()` in server.ts, alle 14 Insert-Actions (kunden/projekte/raeume/partner/produkte/einstellungen/notizen/freigabe-token/timeline/konfigurator/onboarding/team) mit `organisation_id` versehen, `organisation_id?: string | null` in alle relevanten Types-Interfaces eingetragen.
