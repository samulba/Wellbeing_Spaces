...
const testimonials = [
  {
    ...
    quote:
      'Endlich kein Excel-Chaos mehr. Mit Wellbeing Spaces erstelle ich Produktlisten in Minuten und meine Kunden geben per Link frei – ohne eine einzige Rückfrage.',
  },
  {
    name: 'Tobias K.',
    role: 'Architektur Studio, Berlin',
    initials: 'TK',
    color: 'from-wellbeing-green-light to-wellbeing-green',
    quote:
...
export default function Testimonials() {
  return (
    <section className="bg-[#F8F9FA] py-28 relative overflow-hidden">
      {/* Background glow */}
      <div
        className="absolute -top-32 left-1/2 -translate-x-1/2 w-[700px] h-[400px] bg-wellbeing-cream/60 blur-[100px] rounded-full pointer-events-none"
        aria-hidden
      />
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {testimonials.map((t, i) => (
            <AnimateOnScroll key={t.name} delay={i * 130} type="scale-in">
              <div className="bg-white rounded-2xl border border-gray-200 p-7 flex flex-col gap-5 hover:border-wellbeing-green-light hover:shadow-xl hover:shadow-wellbeing-cream hover:-translate-y-1.5 transition-all duration-300 h-full">
                {/* Stars */}
                <div className="flex gap-0.5 text-amber-400 text-[15px]">★★★★★</div>

                {/* Quote mark */}
                <div className="font-serif text-[60px] text-wellbeing-cream leading-none -mb-4 select-none">
                  &ldquo;
                </div>
